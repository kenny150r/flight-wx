import { loadExampleTrack } from "./exampleTrack.js";
import { findExampleForIdent, identCandidates, normalizeHex, normalizeIdent } from "./ident.js";
...
async function fetchJson(url, { signal } = {}) {
  const resp = await fetch(url, { signal: withTimeout(signal) });
  ...
  for (const id of candidates) {
    for (const makeUrl of LIVE_ENDPOINTS) {
      try {
        ...
        const got = normalizeIdent(ac?.flight || "");
        const raw = String(ac?.flight || "").toUpperCase().replace(/\s+/g, "");
        if (raw && !candidates.includes(got) && !candidates.includes(raw)) continue;
...
export async function lookupFlightTrack({ flight, dateClean, hex, signal } = {}) {
  ...
  const example = findExampleForIdent(flight, dateClean);
  if (example) {
    const bundled = await loadExampleTrack(example);
    if (bundled.length) {
      return {
        points: bundled,
        callsign: example.flight,
        hex: hexNorm || null,
        route: route || {
        ...
        notes: [
          `Using the example-library track for ${example.label} (${example.ntsb}). Public historical ADS-B was not available.`,
          ...(example.notes || []),
        ],
      };
    }
...
export { loadExampleTrack } from "./exampleTrack.js";
